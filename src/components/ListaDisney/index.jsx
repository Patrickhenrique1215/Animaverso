...
import styles from "./ListaDisney.module.scss";
import scrollList from "../../utils/snaps.js";

const API_KEY = "d8d845616ef648907b00e45d63d0584f";
const BASE_URL = "https://api.themoviedb.org/3";

function ListaDisney() {

  //States
  const [animacoesDisney, setAnimacoesDisney] = useState([]);
  ...
  //useEffect para esconder e mostrar a seta a esquerda
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchAnimacoesDisney = async () => {
      try {
        ...
        setAnimacoesDisney(todasAnimacoesDisney);
      } catch (error) {
  ...
  //  Render condicional
  if (loading) {
    return <section className={styles.listaDisney}>Carregando...</section>;
  }
  ...
  return (
    ...
    <section ref={containerRef} onScroll={checkScroll} className={styles.listaDisney}>
      {animacoesDisney.filter(animacao => animacao.poster_path).map((animacao) => (
        <a
          key={animacao.id}
