...
import styles from "./ListaFantasia.module.scss";
import scrollList from "../../utils/snaps.js";

const API_KEY = "d8d845616ef648907b00e45d63d0584f";
const BASE_URL = "https://api.themoviedb.org/3";

function ListaFantasia() {

  //States
  const [animacoesFantasia, setAnimacoesFantasia] = useState([]);
  ...
  //useEfect para esconder e mostrar seta a esquerda
  useEffect(() => {
  ...
  //useEffect para buscar animaçoes FANTASIA
  useEffect(() => {
    const fetchAnimacoesFantasia = async () => {
      try {
        // Filmes e séries de animação da Fantasia
        const [filmesRes, seriesRes] = await Promise.all([
          fetch(
            `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=16,14&sort_by=popularity.desc&language=pt-BR`
          ),
          fetch(
            `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_genres=16,14&sort_by=popularity.desc&language=pt-BR`
          )
        ]);
        ...
        const todasAnimacoesFantasia = [
          ...filmes.results,
          ...series.results
        ].slice(0, 30);

        setAnimacoesFantasia(todasAnimacoesFantasia);
      } catch (error) {
        console.error("Erro ao buscar animações da Fantasia:", error);
      } finally {
        setLoading(false);
      }
    };

    fetchAnimacoesFantasia();
  }, []);
  ...
  //Render condicional
  if (loading) {
    return <section className={styles.listaFantasia}>Carregando...</section>;
  }
  ...
  return (
    ...
    <section ref={containerRef} onScroll={checkScroll} className={styles.listaFantasia}>
      {animacoesFantasia.filter(animacao => animacao.poster_path).map((animacao) => (
        <a
          key={animacao.id}
