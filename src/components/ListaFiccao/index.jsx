...
import styles from "./ListaFiccao.module.scss";
import scrollList from "../../utils/snaps.js";

const API_KEY = "d8d845616ef648907b00e45d63d0584f";
const BASE_URL = "https://api.themoviedb.org/3";

function ListaFiccao() {

  //States
  const [animacoesFiccao, setAnimacoesFiccao] = useState([]);
  ...
  //useEffewct para esconder e mostrar aa seta a esquerda
  useEffect(() => {
  ...
  //useEffect para buscar animaçoes de FICCÇAO
  useEffect(() => {
    const fetchAnimacoesFiccao = async () => {
      try {
        // Filmes e séries de animação da Ficcao
        const [filmesRes, seriesRes] = await Promise.all([
          fetch(
            `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=16,878&sort_by=popularity.desc&language=pt-BR`
          ),
          fetch(
            `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_genres=16,878&sort_by=popularity.desc&language=pt-BR`
          )
        ]);
        ...
        const todasAnimacoesFiccao = [
          ...filmes.results,
          ...series.results
        ].slice(0, 30);

        setAnimacoesFiccao(todasAnimacoesFiccao);
      } catch (error) {
        console.error("Erro ao buscar animações da Ficcao:", error);
      } finally {
        setLoading(false);
      }
    };

    fetchAnimacoesFiccao();
  }, []);
  ...
  //Render condicional
  if (loading) {
    return <section className={styles.listaFiccao}>Carregando...</section>;
  }
  ...
  return (
    ...
    <section ref={containerRef} onScroll={checkScroll} className={styles.listaFiccao}>
      {animacoesFiccao.filter(animacao => animacao.poster_path).map((animacao) => (
        <a
          key={animacao.id}
